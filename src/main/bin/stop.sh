...
cd "$WORK"
$JAVA $JAVA_OPTS -Xbootclasspath/p:$BOOT_CP -cp $CP -Djava.rmi.server.hostname=$HOST -Dz8.webserver.port=$PORT -Dz8.webserver.webapp=$WEB org.zenframework.z8.server.engine.ServerMain -server webserver -stop